...
def back_black(image_old,boundary_pts,color):
    image = image_old.copy()
    filled_image = np.zeros_like(image)
    cv2.drawContours(filled_image , [boundary_pts], 0 , color , thickness=cv2.FILLED)
    return filled_image


def back_normal(image_old,boundary_pts,color):
    image = image_old.copy()
    
    cv2.drawContours(image , [boundary_pts], 0 , color , thickness=cv2.FILLED)
    return image


def back_white(image_old,boundary_pts,color):
    image = image_old.copy()
...
def alpha_blending(image_old, boundary_pts, alpha,color):
    image = image_old.copy()
...
def lower_contrast(cv2_image, boundary_pts):
    ...
    mask = np.zeros((cv2_image.shape[0], cv2_image.shape[1]), dtype=np.uint8)
    cv2.fillPoly(mask, [boundary_pts], (255,))

    # Use NumPy vectorized operation to divide each channel by 2 for points inside the boundary
    cv2_image[mask > 0] //= 2
...
def non_linear_lower(cv2_image, boundary_pts):
    ...
    # Use NumPy vectorized operation to apply non-linear transformation
    cv2_image[mask > 0] = ((cv2_image[mask > 0] / 255) ** (1/3)) * 255
...
def invert(cv2_image, boundary_pts):
    ...
    # Use NumPy vectorized operation to invert the pixel values
    cv2_image[mask > 0] = 255 - cv2_image[mask > 0]
